...
const TourDetails = () => {  // Removed 'tour' prop since it's not used
  const { id } = useParams();
  ...
  const { photo, title, desc, price, city, distance, maxGroupSize, featured, address } = tourDetails;
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentUser !== null) {
      ...
      try {
        await axios.post(`https://traveltrek.onrender.com/reviews/${id}`, 
          { username, reviewText, rating, time }, 
    ...
    <div className="md:ml-6 md:mr-6 m-2 shadow-2xl flex md:flex-row flex-col font-shantell font-bold">
      <div className="bg-white shadow-lg overflow-hidden md:w-2/3">
        <div className="relative">
          <img className="h-96 object-cover object-center w-full" src={photo} alt="Tour Image" />
          {featured && (
            <span className="absolute bg-yellow-300 bottom-0 right-0 p-1 rounded-sm">Featured</span>
          )}
        </div>
        <div className="p-4 m-6">
          <div className="flex items-center justify-between mb-4 mr-6">
            ...
            <span className="block text-amber-600 text-bold text-xl">
              ...
              {tourReviews.length === 0 ? "Not Rated" : <span>({tourReviews.length}) Ratings</span>}
            </span>
          </div>
          <p className="text-gray-700 text-base mb-4">{desc}</p>
          <div className="flex flex-row flex-wrap justify-between">
            <div className="p-2"><i className="ri-map-pin-user-fill"></i> {address}</div>
            <div className="p-2"><i className="ri-map-pin-2-line"></i> {city}</div>
            <div className="p-2"><i className="ri-group-line"></i> {maxGroupSize} people</div>
            <div className="p-2"><i className="ri-map-pin-user-fill"></i> {distance} km</div>
          </div>
          <div className="flex flex-col flex-wrap justify-between pb-4">
            ...
            <form onSubmit={handleSubmit}>
              <div className="flex border-pink-200 shadow-sm rounded-lg ">
                ...
                <button type="submit" className="btn bg-teal-400 rounded-md ml-4 pl-3 pr-3 my-1">
                  Share
                </button>
              </div>
            ...
            <div className="user-reviews container flex flex-col flex-wrap">
              {tourReviews.map((review) => (
                <div className="mt-3 shadow-md flex flex-wrap flex-col p-3" key={review._id}>
                  <div className="flex items-center flex-row mr-4">
                    <img className="h-10 w-10 rounded-full" src={avatar} alt="Reviewer" />
                    <div className="text-sm font-medium text-gray-900 mr-5">{review.username}</div>
